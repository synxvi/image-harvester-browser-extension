...
class DomainExclusions {
    ...
    async loadExclusions() {
        try {
            ...
            this.showStatus(chrome.i18n.getMessage('exclusionsLoadFailed') || 'Failed to load exclusions', 'error');
        }
    }
    ...
    async saveExclusions() {
        try {
            ...
            if (success) {
                ...
                this.showStatus(chrome.i18n.getMessage('exclusionsSaveSuccess') || 'Settings saved successfully', 'success');
                this.updateStorageStatus();
            } else {
            ...
            this.showStatus(chrome.i18n.getMessage('exclusionsSaveFailed') || 'Failed to save settings', 'error');
        }
    }
    ...
    validateDomain(domain) {
        if (!domain) {
            return { valid: false, error: chrome.i18n.getMessage('exclusionsErrorEmpty') || 'Domain cannot be empty' };
        }
        ...
        if (!domainRegex.test(domain)) {
            return { valid: false, error: chrome.i18n.getMessage('exclusionsErrorInvalid') || 'Invalid domain format' };
        }

        if (this.exclusions.includes(domain)) {
            return { valid: false, error: chrome.i18n.getMessage('exclusionsErrorDuplicate') || 'Domain already excluded' };
        }
    ...
    renderExclusions() {
        ...
        if (this.exclusions.length === 0) {
            ...
            const removeLabel = chrome.i18n.getMessage('exclusionsRemoveBtn') || 'Remove';
    ...
    updateStorageStatus() {
        ...
        storageElement.textContent = chrome.i18n.getMessage('exclusionsSyncStatus') || '☁️ Settings will be synced across your devices';
    }
